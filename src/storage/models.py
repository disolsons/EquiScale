...
from src.storage.db_setup import Base
from sqlalchemy import (
    Column,
    Integer,
    String,
...
class RawStatementFact(Base):
    """
    Persisted raw statement fact at one-row-per-period grain.
    """
    __tablename__ = "raw_statement_facts"
    ...
    parent_raw_tag = Column(String, nullable=True)
    parent_label = Column(String, nullable=True)
    calculation_weight = Column(Float, nullable=True)

    source_layer = Column(String, nullable=True, index=True)
    source_concept = Column(String, nullable=True)
    filing_accession = Column(String, nullable=True)
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "period_label",
            "source_layer",
            name="uq_raw_statement_fact_row",
        ),
    )
